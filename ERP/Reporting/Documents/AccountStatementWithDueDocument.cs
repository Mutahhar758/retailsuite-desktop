...
namespace ERP.Reporting.Documents
{
    ...
    public class AccountStatementWithDueDocument : IDocument
    {
        ...
        private void ComposeHeader(IContainer container)
        {
            ...
            {
                // Top Identity & Period Header (Clean, institutional layout)
                col.Item().Row(row =>
                {
                ...
                // Pure Account Title (No cluttered boxes)
        ...
        private void ComposeContent(IContainer container)
        {
            container.PaddingTop(8).Table(table =>
            {
                table.ColumnsDefinition(columns =>
                {
                    ...
                    columns.ConstantColumn(56);  // Voucher No
                    columns.RelativeColumn(3);   // Particulars
                    columns.ConstantColumn(48);  // Due Days
                    columns.ConstantColumn(62);  // Due Date
        ...
        /// </summary>
        public static async Task<string> GeneratePdfToTempFileAsync(AccountStatementWithDueHeader header, List<AccountStatementWithDueReportItem> items)
        {
            return await Task.Run(() =>
            {
                ...
                string tempPath = Path.Combine(tempDir, string.Format("AccountStatementWithDue_{0:yyyyMMdd_HHmmss}_{1}.pdf", DateTime.Now, Guid.NewGuid().ToString("N").Substring(0, 6)));
